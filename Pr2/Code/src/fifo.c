...
int mutexDestroyFifo(){
    ...
    err(errno, "fehler bei mutexDestroyFifo1\n",false);
    errno = sem_destroy(&underflow);
    err(errno, "fehler bei mutexDestroyFifo2\n",false);
    errno = pthread_cond_destroy(&full);
    err(errno, "fehler bei mutexDestroyFifo3\n",false);
    errno = pthread_cond_destroy(&empty);
    err(errno, "fehler bei mutexDestroyFifo4\n",false);
    errno = pthread_mutex_destroy(&mutex);
    err(errno, "fehler bei mutexDestroyFifo5\n",true); // BEI DESTROY FIFO FOLGT FATA ERROR
    return 0;
    }

//Init
int initFifo(){
    errno = sem_init(&overflow, 0, FIFO_SIZE); 
    err(errno, "fehler bei Sem_InitFifo\n",true);
    errno = sem_init(&underflow, 0, 0);
    err(errno, "fehler bei mutexDestroyFifoINIT\n",true);
